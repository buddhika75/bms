...
public class MembershipSchemeController implements Serializable {
    ...
    public MembershipScheme findMembershipScheme(String name, Boolean createNew) {
        ...
        if (createNew) {
            tm = new MembershipScheme();
            tm.setName(name);
            tm.setInstitution(getSessionController().getInstitution());
            tm.setDepartment(getSessionController().getDepartment());
            getFacade().create(tm);
        }
        return tm;
    }
    ...
    public MembershipScheme fetchPatientMembershipScheme(Patient patient) {
        ...
        return patient.getPerson().getMembershipScheme();

        //TODO: Patient Membership has NOT expiary
